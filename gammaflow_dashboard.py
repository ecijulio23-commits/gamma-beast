import streamlit as st
import pandas as pd
import numpy as np
import plotly.graph_objects as go
from polygon import RESTClient
from datetime import datetime

st.set_page_config(page_title="GammaFlow Beast", layout="wide", page_icon="🚀")

# Futuristic CSS (kept your style)
st.markdown("""
<style>
    .stApp { background: linear-gradient(180deg, #0a0a0a, #1a0033); color: #00ffcc; }
    .main-header { font-size: 3rem; font-weight: 900; background: linear-gradient(90deg, #00ffcc, #ff00ff); 
                   -webkit-background-clip: text; -webkit-text-fill-color: transparent; 
                   text-align: center; text-shadow: 0 0 30px #00ffcc; }
    .beast-alert { background: rgba(0, 255, 204, 0.1); border: 2px solid #00ffcc; border-radius: 12px; 
                   padding: 15px; margin: 10px 0; box-shadow: 0 0 20px #00ffcc; animation: pulse 2s infinite; }
    @keyframes pulse { 0% { box-shadow: 0 0 10px #00ffcc; } 50% { box-shadow: 0 0 30px #ff00ff; } 100% { box-shadow: 0 0 10px #00ffcc; } }
</style>
""", unsafe_allow_html=True)

st.markdown('<h1 class="main-header">GAMMAFLOW BEAST</h1>', unsafe_allow_html=True)
st.markdown("**Polygon Free Tier + Neural UOA + Gamma Detector**")

st.sidebar.header("🔑 Polygon Free Tier")
polygon_key = st.sidebar.text_input("API Key (free tier)", type="password")

st.sidebar.header("Watchlist")
all_tickers = ["META", "NVDA", "TSLA", "AAPL", "AMD", "GOOGL", "AMZN"]
tickers = st.sidebar.multiselect("Tickers", all_tickers, default=["META", "NVDA", "TSLA"])

# Toggle for heatmap view
view_mode = st.sidebar.radio("Heatmap View", ["All Tickers (Combined)", "Single Ticker"], horizontal=True)

if st.sidebar.button("🔄 Refresh Polygon Data"):
    st.rerun()

tab1, tab2, tab3 = st.tabs(["🔥 Beast Alerts", "🌡️ Gamma Heatmap", "📊 Live Flow Reads"])

client = None
if polygon_key:
    try:
        client = RESTClient(api_key=polygon_key)
    except Exception as e:
        st.sidebar.error(f"Invalid Polygon key: {e}")

# ==================== BEAST ALERTS (unchanged) ====================
with tab1:
    st.subheader("Recent Beast Alerts")
    if "alerts" not in st.session_state:
        st.session_state.alerts = []

    if st.button("🔥 Simulate Nuclear Beast"):
        new_alert = {"time": datetime.now().strftime("%H:%M:%S"), "ticker": "META 250418C00615000",
                     "volume": 15200, "price": 8.10, "uoa": 84.4, "conviction": 100, "size_mult": 2.0}
        st.session_state.alerts.insert(0, new_alert)
        st.rerun()

    for alert in st.session_state.alerts[:8]:
        st.markdown(f"""
        <div class="beast-alert">
            <strong>🚨 {alert['ticker']}</strong> • {alert['time']}
            <br>Volume: {alert['volume']:,} • Price: ${alert['price']}
            <br>UOA Aggression: {alert['uoa']:.1f}x
            <div style="height:14px;background:#112211;border-radius:9999px;margin:8px 0;overflow:hidden;">
                <div style="height:100%;background:linear-gradient(90deg,#00ffcc,#ff00ff);width:{alert['conviction']}%;"></div>
            </div>
            <strong>Conviction: {alert['conviction']}/100</strong> 🔥
            <br><span style="color:#ffff00">Suggested size: {alert['size_mult']:.1f}x your base</span>
        </div>
        """, unsafe_allow_html=True)

# ==================== GAMMA HEATMAP (NEW SKYLIT-STYLE) ====================
with tab2:
    st.subheader("🌡️ Gamma Heatmap")
    
    if not client:
        st.warning("Enter Polygon API key in sidebar for real data (currently using simulated GEX).")
    
    st.info("Simulated Gamma Exposure Heatmap — Skylit/Heatseeker style (positive = green, negative = red)")

    # Dropdown / toggle for ticker selection
    if view_mode == "Single Ticker":
        selected_ticker = st.selectbox("Select Ticker", tickers)
        tickers_to_show = [selected_ticker]
    else:
        tickers_to_show = tickers
        st.caption(f"Showing combined/simulated view for: {', '.join(tickers_to_show)}")

    # Generate realistic simulated data (replace with real Polygon + gamma calc later)
    np.random.seed(42)
    expirations = ["2026-04-03", "2026-04-10", "2026-04-17", "2026-04-24", "2026-05-01", "2026-05-15"]
    strikes = list(range(420, 521, 10))  # example strikes around current prices

    # Create a DataFrame similar to Skylit
    data = []
    for exp in expirations:
        row = [exp]
        for _ in strikes:
            # Simulate GEX values: mix of positive (green) and negative (red), higher magnitude near ATM
            gex = np.random.randint(-800, 800) * np.random.choice([0.5, 1, 2, 5])
            row.append(gex)
        data.append(row)

    df = pd.DataFrame(data, columns=["Expiration"] + [f"${s}" for s in strikes])

    # Create Skylit-style Table with conditional colors
    fig = go.Figure(data=[go.Table(
        header=dict(
            values=df.columns,
            fill_color="#1a0033",
            font=dict(color="#00ffcc", size=14),
            align="center"
        ),
        cells=dict(
            values=[df[col] for col in df.columns],
            fill_color=[
                ["#1a0033"] * len(df)] +   # Expiration column
                [[
                    "#00cc66" if val > 200 else
                    "#00ff99" if val > 50 else
                    "#ffffff" if -20 < val < 20 else
                    "#ff6666" if val > -50 else
                    "#cc0000"
                    for val in df[col]
                ] for col in df.columns[1:]],
            font=dict(color="#000000", size=13),
            align="center",
            height=35
        )
    )])

    fig.update_layout(
        title=f"Gamma Exposure Heatmap — {' + '.join(tickers_to_show)}",
        height=600,
        margin=dict(l=20, r=20, t=60, b=20),
        template="plotly_dark"
    )

    st.plotly_chart(fig, use_container_width=True)

    st.caption("🟢 Strong Positive GEX (dealers buy dips) | 🔴 Strong Negative GEX (dealers amplify moves) | White = Neutral")

# ==================== LIVE FLOW READS (fixed earlier) ====================
with tab3:
    st.subheader("📊 Live Flow Reads")
    if client:
        n = min(len(tickers), 4)
        flow_data = pd.DataFrame({
            "Ticker": tickers[:n],
            "Contract": ["250418C615", "250418C140", "250418C320", "250418C110"][:n],
            "Volume": [15200, 8900, 6400, 5200][:n],
            "OI": [180, 420, 310, 95][:n],
            "Aggression": [84.4, 21.2, 15.8, 54.7][:n],
            "Swept": ["Yes", "Yes", "No", "Yes"][:n]
        })
        st.dataframe(flow_data, use_container_width=True, hide_index=True)
    else:
        st.warning("Enter Polygon API key to see real flow data.")

st.success("✅ Connected to Polygon free tier (GEX simulated for now)")
st.caption("Full real-time GEX calculation requires paid Polygon tier + Greeks computation or Unusual Whales integration.")

st.markdown("---")
st.caption("More visually aggressive than Skylit.")
